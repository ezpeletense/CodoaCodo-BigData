...
def check(*args):
    messagebox.showinfo('¡CAMBIO DETECTADO!',
                        f'La variable cambió a "{mi_color.get()}"')
...
# w pide que detecte la escritura y dispare la fx 'check'
mi_color.trace('w', check)

opt_menu = OptionMenu(mi_frame, mi_color, *colores, command=colorear)
opt_menu.config(state='normal')
opt_menu.pack()
